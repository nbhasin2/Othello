...
public class OthelloConsole extends GameConsole {
	...
	private static final int SUBBLACK = -2;
	private static final int SUBEMPTY = 0;
		...
		Runnable myRunnable = new Runnable() {
			...
			public void run() {
				...
				do {
					...
					evaluate();
					// Rest the redo board array because now player has made a
					// move and previous undo no longer exists.
	...
	public void playOthelloEventBased() {
		gameStateModel.setCurrentBoard(board.makeDeepCopy());
		System.out.println("Current Player - " + currentPlayer);
		playerMove(currentPlayer);
		evaluate();
		...
		gameStateModel.setCurrentBoard(board.makeDeepCopy());
		// currentPlayer = (currentPlayer == SharedConstants.PlayableItem.BLACK)
	...
	private void playerMove(SharedConstants.PlayableItem move) {
		...
		do {
			...
			if (move == SharedConstants.PlayableItem.BLACK) {
				...
				if (!(getAvailableSolutions(SUBBLACK).size() == SUBEMPTY)) {
			...
			int substituteLevel = (move == SharedConstants.PlayableItem.BLACK ? -2
					: -1);
	 ...
	 */
	private int validMove(int row, int col,
